...
import com.to.game.puzzle.kids.model.*;
...
public class ImageCutter extends AppCompatActivity {
    ...
    public PuzzlePiece[][] cutImage() {
        ...
        for (int i = 0; i < cutMap.getHorizontalResolution(); i++) {
            for (int j = 0; j < cutMap.getVerticalResolution(); j++) {
                ...
                Rect pieceBorder = PuzzlePieceBorderFinder.INSTANCE.getBorders(pieceCurveSet,
                        new Point(i * horizontalStep, j * verticalStep), horizontalStep, verticalStep);
                Pair<Integer, Integer> pieceOffset = PuzzlePieceBorderFinder.INSTANCE.getOffsets(pieceCurveSet, horizontalStep, verticalStep);
    ...
    public Bitmap drawPuzzleCuttingOverlay(Paint paint) {
        ...
        for (int i = 0; i < cutMap.getHorizontalResolution(); i++) {
            for (int j = 1; j < cutMap.getVerticalResolution(); j++) {
                ...
                CurveDrawer.INSTANCE.drawHorizontalCurve(horizontalCurve, overlayCanvas, startPoint, horizontalScale, verticalScale, paint);
            }
        }
        ...
        for (int i = 1; i < cutMap.getHorizontalResolution(); i++) {
            for (int j = 0; j < cutMap.getVerticalResolution(); j++) {
                ...
                CurveDrawer.INSTANCE.drawVerticalCurve(verticalCurve, overlayCanvas, startPoint, horizontalScale, verticalScale, paint);
            }
        }
    ...
    public Bitmap drawSinglePuzzlePieceMask(int xIndex, int yIndex, PieceCurveSet pieceCurveSet) {
        ...
        CurveDrawer.INSTANCE.drawPuzzlePiece(canvas, pieceCurveSet, startPoint, currentHorizontalStep, currentVerticalStep, cutterPaint);
